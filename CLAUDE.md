...
（2026-07-13/14）數位環島認證：GPS 佐證「地標徽章」（四極點燈塔＋太魯閣口＋清水斷崖）、
　　　　　「縣市收集徽章」（內政部縣市界 GeoJSON＋point-in-polygon，lib/counties.ts）、
　　　　　「環島完賽證書」（規則從寬，lib/certification.ts；html2canvas 匯出、蓋租車品牌 icon）、
　　　　　完賽慶祝彈窗；於「結束整趟旅程」自動判定（/api/trips/complete）→ /achievements 頁。
　　　　　migration 0018 = achievements 表（device 綁定、RLS 鎖 service_role）。
（2026-07-14）環島指南參考區塊（/resources）：環島1號線資訊＋行前準備＋騎前檢查＋國際旅客，
　　　　　忠實引用觀光署 taiwanbike.tw 並標註來源（原 PDF 為圖片檔無可擷取文字，故採網站來源）。
```
...
- DB migrations 已至 0018（0018 = achievements 環島認證表）；**所有 DDL 由 Jimmy 貼進 Supabase SQL Editor**，Claude 不能跑 DDL
